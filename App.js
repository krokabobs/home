...
class Animation extends React.Component {
    ...
    updateAnimationState() {
        this.setState(prevState => ({  var v; function calculate() {
                if (!count) return;
                for (var s = 1; s <= spf; s++) {
                    var f = -C * (rect.x - x0);
                    v += f / m * dt;
    ...
    render() {
        return <Canvas vel={this.state.vel} />
    }
}
...
class CanvasComponent extends React.Component {
    ...
    componentDidMount() {
            var canvas = this.spring_canvas.canvasRef.current;
            //canvas.onselectstart = function () {return false;};      // запрет выделения canvas
            var ctx = canvas.this.refs.getContext("2d");                      // на ctx происходит рисование
    ...
    render() {
        return (
            <canvas id="spring_canvas" width="600" height="100" style="border:1px solid #000000;" ref={this.canvasRef}> </canvas>
        );
    }
}
